...
for i in $split; do echo $i; done | parallel ../../LDhot/ldhot --seq `echo $breed`_ECA`echo $chrom`_{}.ldhat.sites --loc `echo $breed`_ECA`echo $chrom`_{}.ldhat.locs --lk ../`echo $breed`_lk.txt --res `echo $breed`_ECA`echo $chrom`_{}res.txt --out `echo $breed`_ECA`echo $chrom`_{}
